...
- (void)viewDidLoad {
    ...
    self.title = @"登录";
    self.currentEmail = @"songzhiqing213@gmail.com";
...
- (void)setupUI {
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [self.emailLoginButton.leadingAnchor constraintEqualToAnchor:self.googleLoginButton.leadingAnchor],
        [self.emailLoginButton.trailingAnchor constraintEqualToAnchor:self.googleLoginButton.trailingAnchor],
...
- (void)emailLoginTapped {
    ...
    [SUPSDK_M PostLoginWithAccount:self.currentEmail
                              type:0
                          password:[self md5String:@"Poclink918$&@"]
                              code:nil
                            callBack:^(NSURLSessionDataTask * _Nonnull task, SUPLoginModel * _Nullable model, NSString * _Nullable errorStr) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (model && !errorStr) {
